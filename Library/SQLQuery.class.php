...
class SQLQuery {
     ...
     */
    function connect($address, $username, $password, $dbname) {
        $this->_dbHandle = @mysql_connect($address, $username, $password);
        if ($this->_dbHandle != 0) {
...
function search() {
    ...
    /*
     * Getting and logging error
     */
    if (!$this->_result) {
        getError(4, $this->_query);
    }
